...
void csv_writer::post_terminate(simulation& sim)
{
    ...
    if (plotConfig) {
        ...
        std::filesystem::path plotter("cosim_plotter.py");
        ...
        std::stringstream ss;
        ss << "python cosim_plotter.py \"" << path_.string() << "\" \"" << plotConfig->string() << "\"";
...
void csv_config::load(const std::filesystem::path& configPath)
{
    ...
    const auto root = doc.child("cosim:LogConfig");
    const auto components = root.child("cosim:components");
...
fig_id = 0
namespaces = {"cosim": "http://github.com/Ecos-platform/libcosim/resources/schema/ChartConfig"}


def make_tag(tag: str) -> str:
    return "{" + namespaces["cosim"] + "}" + tag

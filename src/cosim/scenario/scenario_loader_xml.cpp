...
#include "cosim/scenario/scenario.hpp"
...
void cosim::load_scenario(simulation& sim, const std::filesystem::path& config)
{
    ...
    const auto root = doc.child("cosim:Scenario");
    for (const auto& action : root) {
        ...
        for (const auto& variable : action) {
            ...
            pugi::xml_node var;
            if ((var = variable.child("cosim:real"))) {
                ...
                sim.invoke_at(
                    ...
                    eps);
            } else if ((var = variable.child("cosim:integer"))) {
                ...
                sim.invoke_at(
                    ...
                    eps);
            } else if ((var = variable.child("cosim:boolean"))) {
                ...
                sim.invoke_at(
                    ...
                    eps);
            } else if ((var = variable.child("cosim:string"))) {

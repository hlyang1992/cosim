...
int main(){
    ...
    const std::filesystem::path fmuDir = std::string(DATA_FOLDER) + "/fmus/dp_ship";
    try {
        ss.add_model("DPController", fmuDir / "DPController.fmu");
        ss.add_model("Observer", fmuDir / "NLPobserver.fmu");
        ss.add_model("ReferenceGenerator", fmuDir / "ReferenceGenerator.fmu");
        ss.add_model("ThrustAllocation", fmuDir / "ThMPC.fmu");
        ss.add_model("Ship", fmuDir / "OSOM.fmu");
        ...
        auto p = sim->get_real_property("Observer::y_hat[1]");
